...
require_once('connect.php');
//data from the form stock.php is collected in the respective variables
    $stock1 = $_GET['stock1'];
	$stock2 = $_GET['stock2'];
	$id = $_GET['id'];
	$pid = $_GET['pid'];
	...
	// the stock_warehouse is updated with the value3 when stock is being moved to a store
	$q = "update stock_warehouse set stock_product_qtty = $value3 where stock_id = $id ";	
...
if (PEAR::isError($res)){
    ...
    die ($res->getMessage(). $res->getUserInfo());
        ...
        else( $q2 = "update stores set store_stock = store_stock + $value2 where product_id = $pid and store_pr_id = $sp");{
		//the sql statment is executed and checked for errors 
        $res2= $db->query($q2);
