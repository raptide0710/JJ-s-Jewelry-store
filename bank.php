...
<h1>view users banking details</h1>
<?php
//start of session 
session_start();
...
<?php  
//the sql statment is placed in a vairable
// the tables users,bank,banking_user_add,users_banking_details are selected and the foreign keys are linked to the primary keys to call the data from the table containing the primary key

$q ="select * from users,bank,banking_user_add,users_banking_details 
where users.users_id = users_banking_details.users_id 
and bank.bank_id = users_banking_details.bank_id 
and  banking_user_add.bank_user_add_id = users_banking_details.bank_user_add_id";
